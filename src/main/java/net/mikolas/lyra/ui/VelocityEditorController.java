...
public class VelocityEditorController {

    @FXML private VBox partsContainer;
    @FXML private net.mikolas.lyra.ui.control.VelocityTriangle velocityTriangle;
    ...
    public void setMultiPatch(MultiPatch multi) {
        ...
        velocityTriangle.setParts(javafx.collections.FXCollections.observableArrayList(multi.getParts()));
        ...
        // Auto-activate first part if none active
        if (!multi.getParts().isEmpty() && getActivePart() == null) {
            multi.getParts().get(0).setActive(true);
        }
    }

    private MultiPart getActivePart() {
        if (currentMulti == null) return null;
        return currentMulti.getParts().stream().filter(MultiPart::isActive).findFirst().orElse(null);
    }
    ...
    private HBox createPartRow(int id, MultiPart part) {
        ...
        RangeSlider slider = new RangeSlider(0, 127, part.getVelLow(), part.getVelHigh());
        HBox.setHgrow(slider, Priority.ALWAYS);
        slider.setPrefHeight(24);

        slider.lowValueProperty().bindBidirectional(part.velLowProperty());
        slider.highValueProperty().bindBidirectional(part.velHighProperty());
